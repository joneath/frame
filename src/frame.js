(function(root, factory) {
  ...
  Frame.App = Backbone.Model.extend({
    defaults: {
      modelPath: 'models/',
  ...
  var appModels = Frame._appModels = {};
  var modelManager = Frame.modelManager = {
    ...
    destroy: function(identifier) {
      var model = appModels[identifier];
      model.off('change', this._updateTTL);
      delete appModels[identifier];
    },
  ...
  var appCollections = Frame._appCollections = {};
  var collectionManager = Frame.collectionManager = {
    get: function(type, identifier, models, options) {
      var id = type + ':' + identifier,
          collection = appCollections[id];

      if (collection && collection._TTL <= Date.now()) {
        collection = null;
      }
      if (!collection) {
        collection = collectionFactory(type, models, options);
    ...
    set: function(identifier, collection) {
      appCollections[identifier] = collection;
      // 5 minute TTL
    ...
    destroy: function(identifier) {
      var collection = appCollections[identifier];
      collection.off('change', this._updateTTL);
      delete appCollections[identifier];
    },
